...
const TrackedProductList = () => {
  ...
  return (
    <div className="tracked-product-list">
      <h2 className="tracked-product-list">Tracked Products</h2>
      <ul>
        {trackedProducts.map((product) => (
          <li key={product.id} className="tracked-product-list-item">
            <label htmlFor={`tracked-product-${product.id}`}>
          {product.name}
        </label>
        <input
          type="checkbox"
          id={`tracked-product-${product.id}`} // Add unique ID for label association
          onChange={() => handleToggleTrackedProduct(product.id)}
          checked={product.tracked}
        />
        {product.tracked && ( // Conditionally render "tracked" badge if true
      ...
      <div className="add-tracked-product">
        ...
        />
        <button class='add-tracked-product' onClick={handleAddTrackedProduct}>Add</button>
      </div>
    </div>
  );
};
